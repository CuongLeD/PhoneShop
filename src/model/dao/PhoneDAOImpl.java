...
public class PhoneDAOImpl implements DBManipulateInterface<Phone>{
	...
	public boolean updateElement(Phone e) {
		...
		try
		{
			String sqlQuery = "UPDATE Phones "
					+ "SET phoneName=?, phoneQuantity=?, phonePrice=?, phoneDescription=?, shopId=?, categoryId=?, vendorId=? "
					+ "WHERE phoneId=?";
			PreparedStatement preparedStatement = connection.prepareStatement(sqlQuery);
			preparedStatement.setString(1, e.getPhoneName());
			preparedStatement.setInt(2, e.getQuantity());
			...
			for(ImageLink image : e.getImageLinks())
			{
				new ImageLinkDAOImpl().updateElement(image);
			}
			if(affectRows > 0)
				return true;
		}catch(SQLException ex)
	...
	public Phone getElementById(int id) {
		...
		try
		{
			String sqlQuery = "SELECT  phoneId, phoneName, phoneQuantity, phonePrice, phoneDescrible, Phones.shopId, Phones.categoryId, Vendors.vendorId, Vendors.vendorName, Shops.shopName, Categories.categoryName "
					+ "FROM Phones "
